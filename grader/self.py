...
def check_futex() -> List[Check]:
    exact_output2 = 'Hello WoHello World!    rld!    Hello WoHello World!    rld!    '
    
    return check_execution('./selfie -c <assignment>1-wait-1-wake.c -x 2',
                        '2 processes are printing in sequential order with the use of futexes on MIPSTER',
                        success_criteria='Hello World!    ' * 2) + \
        check_execution('./selfie -c <assignment>2-wait-2-wake.c -x 4',
                        '4 processes are printing with exact interleaved output',
                        success_criteria=lambda code, out: (True, '') if exact_output2 in out else (False, f'Expected exact output, got: {out}'))
...
assignment_processes = Assignment('processes', 'Systems', 'processes',
           ...
           check_processes)
assignment_futex = Assignment('futex', 'Systems', 'futex',
           REPO_BLOB_BASE_URI + 'grader/systems-assignments.md#assignment-futex',
           check_futex, parent = assignment_processes)
